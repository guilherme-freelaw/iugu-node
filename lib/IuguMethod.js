...
module.exports = function iuguMethod(spec) {
  ...
  return function () {
    ...
    var auth =
      args.length > urlParams.length && utils.isAuthKey(args[args.length - 1]) ? args.pop() : null;
    ...
    // For GET requests, move params to query string and clear body
    if (requestMethod === 'GET' && data && typeof data === 'object') {
      var qsData = {};
      for (var k in data) {
        if (Object.prototype.hasOwnProperty.call(data, k)) qsData[k] = data[k];
      }
      if (qsData.hasOwnProperty('count') && !qsData.hasOwnProperty('limit')) {
        qsData.limit = qsData.count;
        delete qsData.count;
      }
      var qs = utils.stringifyRequestData(qsData);
      if (qs && qs.length > 0) {
        requestPath = requestPath + (requestPath.indexOf('?') === -1 ? '?' : '&') + qs;
      }
      data = {};
    }
