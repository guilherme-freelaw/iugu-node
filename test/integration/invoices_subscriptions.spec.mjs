...
async function testInvoice() {
  const payload = {
    ...
    data: { id: 'int-inv-1', customer_id: 'int-test-cust-1', total_cents: 12345 },
    timestamp: new Date().toISOString()
  };
  const row = await insertEvent({ event_name: payload.event_name, entity_id: payload.data.id, payload, dedupe_key: 'int-inv-' + Date.now(), received_at: new Date().toISOString(), process_status: 'pending' });
  console.log('inserted invoice event', row.id);
...
async function testSubscription() {
  const payload = {
    ...
    data: { id: 'int-sub-1', customer_id: 'int-test-cust-1', plan_id: 'plan-1' },
    timestamp: new Date().toISOString()
  };
  const row = await insertEvent({ event_name: payload.event_name, entity_id: payload.data.id, payload, dedupe_key: 'int-sub-' + Date.now(), received_at: new Date().toISOString(), process_status: 'pending' });
  console.log('inserted subscription event', row.id);
  const proc = runProcessor();
  console.log('processor stdout', proc.stdout);
  console.log('processor stderr', proc.stderr);
  if (proc.status !== 0) throw new Error('processor failed');
